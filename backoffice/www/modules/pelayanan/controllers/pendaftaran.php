...
//        $this->output->cache(-1);
        ...
        if ($data_paralel == "no") {
			...
			$cek_bpjs = $this->cek_bpjsparalel($id_paralel);
			//echo $cek_bpjs;
			//foreach ($id_paralel as $row) {
			//	$row_izin = new trperizinan();
			//	$row_izin->get_by_id($row);
			//	$cek_bpjs[] = $row_izin->cek_bpjs;
			//	}
			}
        $data['id_paralel'] = $id_paralel;
		$data['cek_bpjs'] = $cek_bpjs;
...
//cek Online penduduk
        ...
        $js = "
				var hasil_bpjs = ". $this->value_bpjs($cek_bpjs) .";
                $(document).ready(function() {
                    $('#form').validate();
                    $(\"#tabs\").tabs();
                ...
                function show_npwp(form) {
                     var reg = form.nodaftar_id.value;
				...
				$(document).ready(function() {
					$('#cek_bpjs').on('click', function(){
						
                     var reg = form.nodaftar_id.value;
                     if (reg.length==0)
                    {
                        alert('NPP BPJS Ketenagakerjaan harus diisi!');
                        return false;
                    }
                    else if (reg.length==0)
                    {
                    ...
                    {
					$.ajax({
                                url:'".site_url('pelayanan/pendaftaran/bpjs_ket_cek/')."',
                                type:'GET',
								data:'no_reg_perusahaan=' + $('#nodaftar_id').val(),
                                success:function(response){
									id_number = JSON.parse(response);
									hasil_bpjs = id_number.ret;
									alert(id_number.pesan);
									cheker();
                                },
								error:function(response){
									alert('Gagal terkoneksi ke BPJS ' + response);
								}
                            });
                    return false;
                   }
	...
	public function value_bpjs($value=NULL){
		if($value > "0"){
			return "-1";
		}else{
			return "0";
	...
	public function cek_bpjsparalel($bpjs_paralel=NULL){
		//$bpjs_paralel = $_GET["bpjs"];
		$sql = "SELECT sum(cek_bpjs) FROM trperizinan where id IN ($bpjs_paralel)";
		//$sql = $this->db->query
		$results = $this->db->query($sql);
		foreach ($results->result_array() as $row)
{
        $hasil_cek = $row['sum(cek_bpjs)'];
}
		return $hasil_cek;
	}
...
//        }
    ...
    public function selesai($id_daftar = NULL) {
		...
		$no_pemohon = $pemohon->telp_pemohon;
...
//      $tracking_izin->save($permohonan);
        ...
        $text = "Terima Kasih telah melakukan pendaftaran izin anda pada DPMPPTSP Karo, untuk tracking permohonan balas dengan 'track#".$permohonan->pendaftaran_id."', untuk pengaduan 'tanya#nama anda#pertanyaan/pengaduan'";
		//$this->send_sms($text,$no_pemohon);
...
//      $jam = date("H:i:s A");
    ...
    public function cetak_bukti($id_daftar = NULL) {
		...
		$jenis_permohonan = $permohonan->trjenis_permohonan->get();
...
//        $kota = $this->tr_instansi->get_by_id(11);
		...
		$listeArticles = array(
			...
			array('property' => 'Email',
                'content' => $pemohon->email_pemohon,
            ),
            array('property' => 'Jenis Izin',
                'content' => $data_izin,
            ),
			array('property' => 'Status Permohonan',
                'content' => $jenis_permohonan->n_permohonan,
            ),
...
//            }
	...
	public function bpjs_ket_cek(){
		$cek_bpjs_post = $_GET["no_reg_perusahaan"];
		//echo 'aaa';
		//die();
		//$request_body = file_get_contents('php://input');
		//$cek_req = json_decode($request_body);
		//if (isset($_POST["no_reg_perusahaan"]) && !empty($_POST["no_reg_perusahaan"])) {	 
		//$no_reg_perusahaan = $cek_req->no_reg_perusahaan;
		$ipfwd = getenv('HTTP_X_FORWARDED_FOR');
			if ($ipfwd=='') $ipfwd = getenv('REMOTE_ADDR');
		$phpInternalEncoding = "UTF-8";
		$url = 'https://es.bpjsketenagakerjaan.go.id/api/ceknpp';
		$fields = array(
				'username' => 'user',
				'password' => 'password',
				'npp' => $cek_bpjs_post
		...
		curl_close($ch);
		//echo $result;
		//echo $no_reg_perusahaan; 
		//print_r($result);
		$resultWS = json_decode($result);
		
		//print_r($resultWS->data->namaPerusahaan);
		$hasil_return = array();
		if ($resultWS->ret == "0") {
			$hasil_return = 
				array("ret" => $resultWS->ret,
					"pesan" => 'Nama Perusahaan : '.$resultWS->data->namaPerusahaan.'. Status Peserta: '.$resultWS->data->statusPeserta.'. Pesan : '.$resultWS->msg);
		}else {
		//	print_r($resultWS);
		//	echo '</pre>';
		//	echo '<br><br>';
		//	echo 'Nama Prsh: '.$resultWS->data->namaPerusahaan;
		//	echo '<br>';
		//	echo 'Status Peserta: '.$resultWS->data->statusPeserta;
		//
			$hasil_return = 
				array("ret" => $resultWS->ret,
					"pesan" => $resultWS->msg);
		...
		echo json_encode($hasil_return);
		print $resultWS;
	//}else{
	//	echo "Post tidak sesuai";
	//}
		}
