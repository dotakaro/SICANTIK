	...
	*/
    function list_Monitoring_Per_Perizinan() {
        ...
        if ($obj) {
			...
			//$tgl_permohonanss = GregorianToJD($tgl_permohonans[1],$tgl_permohonans[2],$tgl_permohonans[0]);
			//$durasi = $tgl_skrng - $tgl_permohonanss;

			//if ($durasi >= $lama_pengerjaan->v_hari){
            ...
            foreach ($obj as $list) {
                ...
                $aaData[] = array(
                    ...
                    $list->n_sts_permohonan,
                    $list->a_izin,
                    $this->hitung_hari($list->v_hari, $list->d_terima_berkas, $list->n_sts_permohonan)
	...
	function hitung_hari($hari=NULL, $tanggal_terima=NULL, $statuss=NULL){
		$date2 = date('d');
		$month2 = date('m');
		$year2 = date('Y');
		$tgl_skrng = GregorianToJD($month2, $date2, $year2);
		$tgl_permohonans =  explode('-',$tanggal_terima);
		$tgl_permohonanss = GregorianToJD($tgl_permohonans[1],$tgl_permohonans[2],$tgl_permohonans[0]);
		$durasi = $tgl_skrng - $tgl_permohonanss;
		$hasilperhitungan = "";
		$img_alert = '<img src="'.  base_url() . 'assets/images/icon/warning.png" border="0" onclick="return warning()" style="cursor:pointer;" width="20" height="inherit" />';
		if($durasi >= $hari and $statuss != 'Arsip'){
			$hasilperhitungan = $img_alert;
			
		}else{
			$hasilperhitungan = "Masih sesuai SOP";
		}
    ...
    function get_list_Monitoring_Per_Status($limit=NULL, $start=NULL, $cari=NULL, $first_date=NULL, $second_date=NULL) {
        ...
        if ($cari != NULL) {
            $colom_cari = array(
                "t1.pendaftaran_id", "t3.n_perizinan", "t1.d_terima_berkas", "t5.n_pemohon", "t7.n_sts_permohonan", "t1.a_izin", "t14.nilai_retribusi"
            );
            $sql.=$this->lib_query->add_searching($colom_cari, 'AND', $cari);
        }
