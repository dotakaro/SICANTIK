...
class REST_Controller extends Controller
{
    ...
    public function gantiformathp($nomorhp)
    {
    ...
    public function send_whatsapp($no_hp, $messagetelegram, $files = null, $group = null)
    {
        $this->config->load('whatsapp', TRUE);
        $this->load->library("curl");
        $telegram_location = $this->config->item('whatsapp_location', 'whatsapp') . '/send/file';
        if ($this->config->item('whatsapp_location', 'whatsapp') == 1) {
            $no_hp = $this->gantiformathp($no_hp);
            $messagetelegram = str_replace("<b>", "*", $messagetelegram);
            $messagetelegram = str_replace("</b>", "*", $messagetelegram);
            if (!empty($files)) {
                foreach ($files as $file) {
                    $cFileS1 = curl_file_create($file, 'application/pdf', basename($file));
                    $package1 = array('phone' => $no_hp, "file" => $cFileS1);
                    $this->curl->_curl_telegram($telegram_location, $package1);
                }
            }
            $telegram_location2 = $telegram_location . "/send/message";
            if (empty($group)) {
                $package2 = array('phone' => $no_hp, 'message' => $messagetelegram);
            } else {
                $package2 = array('phone' => $no_hp, 'message' => $messagetelegram, 'type' => "group");
            }
            $this->curl->_curl_telegram($telegram_location2, $package2);
        }
    }

    public function sendEmail($email, $subject, $message, $file = null)
    {
        $this->config->load('mailler', TRUE);
        $this->load->library('phpmailer');
        $mail = new PHPMailer;
        $mail->IsSMTP();
        $mail->SMTPSecure = 'ssl';
        $mail->Host = $this->config->item('smtp_host', 'mailler'); //host email
        $mail->SMTPDebug = 2;
        $mail->Port = $this->config->item('smtp_port', 'mailler');
        $mail->SMTPAuth = true;
        $mail->Username = $this->config->item('smtp_user', 'mailler'); //user email
        $mail->Password = $this->config->item('smtp_pass', 'mailler'); //password email 
        $mail->SetFrom($this->config->item('smtp_user', 'mailler')); //set email pengirim
