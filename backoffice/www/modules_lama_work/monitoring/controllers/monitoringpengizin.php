...
class Monitoringpengizin extends WRC_AdminCont {
    ...
    public function __construct() {
        ...
        foreach ($list_auths as $list_auth) {
            if($list_auth->id_role === '2' or $list_auth->id_role === '8') {
                $enabled = TRUE;
                $this->monitoringpengizin = new user_auth();
            }
        }
    ...
    public function filterdata() {
	 ...
	 $idizin = $this->input->post('ambilizin');
		...
		//$listperusahaan = $this->tmperusahaan->where('id'
 ...
 $list2 = $this->perizinan->where('id',$this->input->post('ambilizin'))->get();
        ...
        $listeArticles3 = array(
                array(	'property' =>  $i,
                        ...
                        'content6' =>  $data_relasi->n_perusahaan,
                        'content7' =>  $data_relasi->a_izin,
...
INNER JOIN tmpermohonan_tmperusahaan ON tmpermohonan_tmperusahaan.tmpermohonan_id = a.id
INNER JOIN tmperusahaan ON tmpermohonan_tmperusahaan.tmperusahaan_id = tmperusahaan.id where a.d_terima_berkas >= '".$tgla."'
