<?php
class Report_table_model extends Model{
    private $_table="report_tables";
...
//		echo "<pre>";print_r($datas);exit();
	...
	public function delete($report_table_id=null){
		$result=false;
		if($report_table_id){
			$this->db->where('id',$report_table_id);
	...
	function get_data($report_table_id=null){
		$data=array();
		if($report_table_id){
			$this->db->where('id',$report_table_id);
			$query=$this->db->get($this->_table);
	...
	function get_all_report_jointable($report_group_data_id=null, $report_generator_id=null){
		$data=array();
		$ret=array();
		$count=0;	
		if($report_group_data_id&&$report_generator_id){
			$this->db->where('report_group_data_id',$report_group_data_id);
			$this->db->where('report_generator_id',$report_generator_id);
			$this->db->where('join_type !=','Main');
			$this->db->where('join_type !=','Direct');
	...
	function get_all_report_table($report_group_data_id=null, $report_generator_id=null){
		$data=array();
		$ret=array();
		$count=0;	
		if($report_group_data_id&&$report_generator_id){
			$this->db->where('report_group_data_id = '.$report_group_data_id." AND report_generator_id = ".$report_generator_id." and ( join_type = 'Main' OR join_type = 'Direct' )");
