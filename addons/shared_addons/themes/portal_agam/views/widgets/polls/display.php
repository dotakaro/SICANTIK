...
<?php if ($is_open AND ! $already_voted): ?>
	...
	<form method="post" action="<?php echo site_url('polls/' . $slug); ?>">
		<fieldset>
			...
			<input type="submit" class="button" name="submit" value="<?php echo lang('polls:vote') ?>" /> 
			
			<a href="<?php echo site_url('polls/' . $slug); ?>">
				<input type="button" class="button" value="<?php echo lang('polls:view_label') ?>" /> 
			</a>
