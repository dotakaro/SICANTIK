...
class Search_index_m extends MY_Model
{
	...
	public function index($module, $singular, $plural, $entry_id, $uri, $title, $description = null, array $options = array())
	{
		...
		// Store a image id from files module of this entry
		if ( ! empty($options['image_id']))
		{
			$this->db->set('image_id', $options['image_id']);
		}
	...
	{
		return $this->db
			->select('title, description, keywords, module, entry_key, entry_plural, uri, cp_edit_uri, image_id')
			...
			->order_by('relevance desc, datetime desc')
			->get('search_index')
			->result();
	}
}
