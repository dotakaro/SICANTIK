 ...
 */
class Widget_Info_singkat extends Widgets
{
	...
	public function run($options)
	{
		...
		$widget_info = $this->db->query(
                        "SELECT * FROM default_info_singkat a 
							WHERE a.published = 1
                            ORDER BY a.created DESC
                            LIMIT {$options['limit']}")->result();
		
		// returns the variables to be used within the widget's view
		return array('widget_info' => $widget_info);
	}
