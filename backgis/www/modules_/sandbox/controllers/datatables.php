...
class Datatables extends WRC_AdminCont {
    ...
    public function getDataTables() {
        $obj = new tmpemohon();
        $obj->start_cache();
        $columns = array('no_referensi', 'n_pemohon', 'a_pemohon');
...
//            }
    ...
    public function getMobileDataTables() {
//        $obj = new tmpemohon();
//        $obj->start_cache();
        $query = $this->db
            ->from('tmpemohon p')
            ->join('mobile_user mu', 'mu.tmpemohon_id = p.id', 'inner')
            ->where('mu.tmpemohon_id IS NOT NULL');

        $obj = $query;
        $columns = array('no_referensi', 'n_pemohon', 'a_pemohon');
//        $this->iTotalRecords = $obj->count();
        $this->iTotalRecords = $obj->count_all_results();
...
//            }
            ...
            if ($i === 0) {
//                $this->iTotalDisplayRecords = $obj->count();
                $this->iTotalDisplayRecords = $obj->count_all_results();
                $obj = $query;
            } else if ($i === 1) {
                if ($this->input->post("iDisplayStart") && $this->input->post("iDisplayLength") != "-1") {
                    $this->iDisplayStart = $this->input->post("iDisplayStart");
...
//                $obj->stop_cache();
                $obj = $this->db->select('p.*, mu.username, mu.active')
                    ->from('tmpemohon p')
                    ->join('mobile_user mu', 'mu.tmpemohon_id = p.id', 'inner')
                    ->where('mu.tmpemohon_id IS NOT NULL')
                    ->order_by('mu.id','DESC');
                echo $this->parseMobilePemohon($obj->get()->result());
//                echo $this->test($obj->order_by('id','desc')->get());
            }
        }
    }
    ...
    private function parseMobilePemohon($obj) {
        ...
        foreach ($obj as $list) {
            ...
            $action .= anchor(site_url('mobile_pemohon/edit') . '/' . $list->id, img($img_edit)) . "&nbsp;";
            if($list->active == 0){
                $action .= anchor(site_url('mobile_pemohon/activate') . '/' . $list->id, img($img_activate)) . "&nbsp;";
            }

            $relasi = new tmpemohon_tmpermohonan();
            $relasi->where('tmpemohon_id', $list->id)->get();
            if (!$relasi->id)
                $action .= anchor(site_url('mobile_pemohon/delete') . '/' . $list->id, img($img_delete)) . "&nbsp;";

            $aaData[] = array(
                $i,
                $list->no_referensi,
                $list->username,
                ($list->active==1)?"Aktif":"Tidak Aktif",
                $list->n_pemohon,
                $list->a_pemohon,
                $action
            );
