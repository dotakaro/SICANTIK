<div id="content">
    <div class="post">
        ...
        <div class="entry">
            <fieldset id="half">
                ...
                <?php echo form_open('rekomendasi');
        ...
        <div class="entry">
        <table cellpadding="0" cellspacing="0" border="0" class="display" id="rekomendasi">
        <thead>
        <tr>
            <th>No</th>
    ...
    //                        if($idkelompok == '2' || $idkelompok == '4'){
                        ...
                        <tr>
                            ...
                            <td><?php echo (isset($mapping_rekomendasi[$rows['rekomendasi']])) ? $mapping_rekomendasi[$rows['rekomendasi']] : '';?></td>
                            <td>
                                <?php
                                $showed = FALSE;
                                $survey_id = NULL;
                            ...
                            <td>
                                ...
                                $img_bukti = array(
                                    ...
                                    'alt' => 'Cetak Rekomendasi Teknis',
                                    'title' => 'Cetak Rekomendasi Teknis',
                                    'border' => '0',
                                );

                                if($rows['c_izin_selesai']==1){
                                    echo anchor(site_url('rekomendasi/view') .'/'. $rows['tim_teknis_id'], img($img_edit))."&nbsp;";
                                }else{
                                    if($rows['idizin'] === '2' || $rows['idizin'] === '3'){
                                        echo anchor(site_url('rekomendasi/edit') .'/'. $rows['tim_teknis_id'], img($img_edit))."&nbsp;";
                                    }else{
                                        echo anchor(site_url('rekomendasi/edit') .'/'. $rows['tim_teknis_id'], img($img_edit))."&nbsp;";
                                    }
                                }
                                if($showed) {
                                    //if($rows['idizin'] === '2' || $rows['idizin'] === '3'){
                                        echo anchor(site_url('report_generator/cetak/REKOMENDASI_TEKNIS') .'/'. $rows['id'].'/'.$rows['idizin'].'/'.$rows['tim_teknis_id'], img($img_bukti),'target="_blank"')."&nbsp;";
                                    //}else{
                                    //    echo anchor(site_url('report_generator/cetak/REKOMENDASI_TEKNIS') .'/'. $rows['id'].'/'.$rows['idizin'].'/'.$rows['tim_teknis_id'], img($img_bukti),'target="_blank"')."&nbsp;";
                                    //}
                                }
