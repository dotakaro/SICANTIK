...
//                ->order_by('id', 'DESC')->get();
		...
		if($sALL==1){
            $query = "SELECT DISTINCT A.id, A.pendaftaran_id, A.c_tinjauan, A.d_terima_berkas, A.d_survey,
                    ...
                    INNER JOIN (select id, tmpermohonan_id, MAX(tmbap_id) as tmbap_id from tmbap_tmpermohonan group by tmpermohonan_id) H ON H.tmpermohonan_id = A.id
...
//     $jam = date("H:i:s A");
       // $p = $this->db->query("call log ('Pembuatan BAP','Update " . $permohonan->pendaftaran_id . "','" . $tgl . "','" . $u_ser . "')");
		$this->rat->log('Save BAP',5,$this->session->userdata('id_auth'),$id_permohonan);
