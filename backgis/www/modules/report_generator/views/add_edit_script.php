<style type="text/css">
    ...
    select#ReportGeneratorTrunitkerjaId{
...
/*	function loadDetailForm(id){
    ...
    function filterPopUp(id){
        var url="<?php echo site_url('report_generator/detail_filter');?>"+'/'+id;
        var browserWidth=$(window).width();
        var browserHeight=$(window).height();
        var width=1024;
        var height=500;
    ...
    $("input[type=button].btn_filter").live('click',function(){
        var report_group_id = $(this).parent().parent().find('td input[type="hidden"].report_group_data_id').val();
        if(report_group_id==''){
            alert("Mohon simpan Report Group Data dahulu sebelum membuka Filter");
        }else{
            filterPopUp(report_group_id);
        }
        return false;
    });
    ...
    function addNewRow(){
		...
		row+='<input type="button" class="btn_filter ui-widget-content" value="Filter" style="display:none"/>';
	...
	$('#download_empty_xml').click(function(){
		var report_generator_id=$('#ReportGeneratorId').val();
		if(report_generator_id!=undefined && report_generator_id !=''){
			window.open(webroot +'report_generator/download_empty_xml/'+report_generator_id,'_blank');
		}else{
			alert('Silahkan tekan tombol simpan terlebih dahulu.');
		}	
	});

    function checkDisplayType(elem, eraseGroupData){
        switch($(elem).val()){
            case 'standalone':
                $('#ReportGeneratorReportType, #ReportGeneratorTrperizinanId, #ReportGeneratorTrunitkerjaId').removeClass('required-option').validate();
                $('#ReportGeneratorLayout').removeClass('required').validate();
                if(eraseGroupData === true){
                    $('#tbl_group tbody').html('');
                    addNewRow();
                }
                $('.report-integrated,#btn_add, #download_empty_xml, .btn_delete').hide();
                $('.btn_filter').show();
                break;
            default:
                $('#ReportGeneratorReportType, #ReportGeneratorTrperizinanId, #ReportGeneratorTrunitkerjaId').addClass('required-option').validate();
                $('#ReportGeneratorLayout').addClass('required').validate();
                if(eraseGroupData === true) {
                    $('#tbl_group tbody').html('');
                }
                $('.report-integrated, #btn_add, #download_empty_xml').show();
                $('.btn_filter').hide();
                break;
        }
    }

    //Jika ada perubahan pada tipe display Report Generator
    $('#ReportGeneratorDisplayType').change(function(){
        checkDisplayType($(this), true);
    });
    ...
    checkDisplayType($('#ReportGeneratorDisplayType'), false);
