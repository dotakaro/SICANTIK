...
class Plugin_Helper extends Plugin
{
	...
	public function date()
	{
		...
		# Translate to global language if date output has a month
		$date_array = array();
		$pieces = explode(" ", $date);
		foreach ($pieces as $key => $value) {
			if ($this->lang->line('global:'.strtolower($value))) {
				$date_array[$key] = $this->lang->line('global:'.strtolower($value));
			} else {
	...
	public function gravatar()
	{
		...
		// Edited
		$default   = $this->attribute('default', false);

		return gravatar($email, $size, $rating, $url_only, $default);
	}
