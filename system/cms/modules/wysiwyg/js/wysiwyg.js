...
function insertImage(file, alt, location, path)
{
	...
	window.parent.instance.insertHtml('<span class="alignment-'+img_float+'" '+float_tag+'>\
		<img '+width_tag+' src="'+path+'" alt="'+alt+'" />\
...
function insertFile(id, title, location, path)
{
	...
	window.parent.instance.insertHtml('<a class="file" href="'+path+'">' + title + '</a>');
    windowClose();
}
