			...
			<table class="table-list" border="0" cellspacing="0">
				...
				<tbody>
					...
					<tr class="<?php echo alternator('', 'alt') ?>">
                        <td class="image">
                        	<?php $img_alt = ($image->alt_attribute) ? $image->alt_attribute : $image->name; ?>
                        	<img class="ib-image" src="<?php echo base_url($image->location === 'local' ? 'files/thumb/'.$image->id.'/50x50/fit' : 'files/cloud_thumb/'.$image->id) ?>" alt="<?php echo $img_alt ?>" width="50" onclick="javascript:insertImage('<?php echo $image->id."', '".htmlentities(addslashes($img_alt))."', '".$image->location."', '".$image->path ?>');" />
                        </td>
