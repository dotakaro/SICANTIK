...
class TestGstr1Spreadsheet(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.company.write({'l10n_in_gst_efiling_feature': True})
    ...
    def test_regenerate_promoted_admin(self):
        self.regular_user.group_ids += self.env.ref("account.group_account_manager")

        self.gstr_return_period.with_user(self.regular_user).generate_gstr1_spreadsheet()
        first_doc = self.gstr_return_period.gstr1_spreadsheet_id
        self.gstr_return_period.with_user(self.regular_user).generate_gstr1_spreadsheet()
        second_doc = self.gstr_return_period.gstr1_spreadsheet_id
