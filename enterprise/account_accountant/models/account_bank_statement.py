...
class AccountBankStatementLine(models.Model):
    ...
    def _try_auto_reconcile_statement_lines(self, company_id=None):
       ...
       LEFT JOIN LATERAL (
        ...
        for st_line_id, aml_id, aml_amount_residual, matching_word in self._cr.fetchall():
        ...
        for st_line_id, all_aml_ids, total_residual in self._cr.fetchall():
            st_line = self.browse(st_line_id).with_prefetch(self._prefetch_ids)  # guarantees batch prefetching if needed
            if total_residual == st_line.amount:
                ...
                if candidate_amls:
                    st_line.with_company(st_line.company_id).set_line_bank_statement_line(candidate_amls.ids)
