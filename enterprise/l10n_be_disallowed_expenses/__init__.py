...
def _l10n_be_disallowed_expenses_post_init(env):
    for company in env['res.company'].search([('chart_template', 'in', ('be_comp', 'be_asso')), ('parent_id', '=', False)]):
        ...
        disallowed_expenses_accounts = {
            xmlid: vals
            for xmlid, vals in Template._get_be_disallowed_expenses_accounts().items()
            if Template.ref(xmlid, raise_if_not_found=False)
        }
        if disallowed_expenses_accounts:
            Template._load_data({
                'account.account': disallowed_expenses_accounts,
            })
